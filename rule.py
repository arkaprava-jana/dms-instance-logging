...
Description:
  Checks whether AWS DMS Replication Task has CloudWatch Logging enabled
...
Scenarios:
  Scenario: 1
    Given: No AWS DMS Replication Task exists
     Then: Return Empty list
  Scenario: 2
    Given: At least one AWS DMS Replication Task exists
      And:  CloudWatch Logging is not enabled for the task
      Then: Return NON_COMPLIANT with annotation "This AWS DMS Replication Task does not have CloudWatch Logging Enabled"
  Scenario: 3
    Given: At least one AWS DMS Replication Task exists
      And: CloudWatch Logging is enabled for the task
...
DEFAULT_RESOURCE_TYPE = 'AWS::DMS::ReplicationTask'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    dms_tasks = client.describe_replication_tasks()['ReplicationTasks']
    evaluations = []
    ...
    for dms_task_details in dms_tasks:
        dms_log_compliance = False
        dms_task_settings = dms_task_details['ReplicationTaskSettings']
        if json.loads(dms_task_settings)['Logging']['EnableLogging'] is True:
            dms_log_compliance = True

        if dms_log_compliance is False:
        #SCENARIO 2
            evaluations.append(build_evaluation(dms_task_details['ReplicationTaskIdentifier'], 'NON_COMPLIANT', event, annotation='This AWS DMS Replication Task does not have CloudWatch Logging Enabled'))
        else:
            #SCENARIO 3
            evaluations.append(build_evaluation(dms_task_details['ReplicationTaskIdentifier'], 'COMPLIANT', event))
